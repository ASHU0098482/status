...
namespace KittyMemory {
    ...
    Memory_Status KittyMemory::memRead1(void *buffer, const void *addr, size_t len) {
        if (addr == NULL)
            return INV_ADDR;
    ...
    static ProcMap1 findMapInCache(std::string id){
        ProcMap1 ret;
    ...
    ProcMap1 KittyMemory::getLibraryMap(const char *libraryName) {
        ProcMap1 retMap;
        char line[512] = {0};
