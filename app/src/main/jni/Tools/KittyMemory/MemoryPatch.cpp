...
MemoryPatch MemoryPatch::createWithHex1(const char *libraryName, uintptr_t address,
    ...
    KittyMemory::memRead1(&patch._orig_code[0], reinterpret_cast<const void *>(patch._address),
                         patch._size);
    return patch;
}

MemoryPatch MemoryPatch::createWithHex1(uintptr_t absolute_address, std::string hex) {
    MemoryPatch patch;
